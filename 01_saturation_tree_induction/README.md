# Research line 01 — Global saturation and maximum-evidence tree induction (v1.1 – v1.4)

**Direction.** Discover anonymous string equivalence by *global* substitution
saturation over a finite observed string universe, and induce unlabeled
projective binary structure by maximizing raw substitution-witness evidence.
This is the original SCF core (v1.1), its synthetic benchmark and gold
evaluator (v1.2), the identifiability audit (v1.2.1), the debiased evidence
objectives and real-corpus constraint audit (v1.3), and the context-indexed
local equivalence relations (v1.4).

**Status.** Complete and frozen; retained as the reference implementation and
regression baseline. Later lines (02–05) do not depend on this code. Its
central negative finding — strict global equivalence collapses under lexical
ambiguity and exact contexts stay >90 % singleton on real text — motivated the
change of direction in lines 02–05 towards *category* recovery from bounded
contextual behaviour.

| what | where |
|---|---|
| public headers | `include/scf/` |
| implementation | `src/` (`main.cpp` is the `scf_cli` entry point) |
| tools | `tools/` (`scf_generate`, `scf_experiment`, `scf_audit`, `scf_real_audit`, `scf_prepare_text`, `scf_synthetic_generator`) |
| regression suite | `tests/test_main.cpp` (`scf_tests`) |
| controlled corpora | `data/synthetic/` |
| default CLI configuration | `config/default.cfg` |
| reports | `reports/IMPLEMENTATION_NOTES.md`, `reports/SCF_v1_2_1_AUDIT.md`, `reports/SCF_v1_3_REPORT.md`, `reports/SCF_v1_4_REPORT.md` |
...
The remainder of this file is the original documentation of the line
(build, research semantics, v1.2 benchmark, v1.2.1 audit, v1.3 objectives,
v1.4 context indexing), with paths updated to the per-line layout.
...
```bash
build/01_saturation_tree_induction/scf_cli --input 01_saturation_tree_induction/data/synthetic/simple.txt --stats --dump-classes --dump-witnesses --dump-evidence --dump-trees
```

On Linux or a single-configuration Windows generator, the executable is normally `build/01_saturation_tree_induction/scf_cli`. The independent generator recreates all supplied controlled corpora:

```bash
scf_synthetic_generator 01_saturation_tree_induction/data/synthetic
```
...
```bash
build/01_saturation_tree_induction/scf_generate --grammar nested_balanced --coverage 0.4 --seed 42 \
...
```bash
build/01_saturation_tree_induction/scf_cli --input data/generated/.../corpus.txt \
...
```bash
build/01_saturation_tree_induction/scf_experiment --grammar simple_np_vp \
...
The full audit lives in `reports/SCF_v1_2_1_AUDIT.md`; headline results:
...
```bash
build/01_saturation_tree_induction/scf_audit --output-dir results/audit_v1_2_1 --seeds 20
```

### Real-data smoke test

```bash
build/01_saturation_tree_induction/scf_prepare_text --input raw.txt --output corpus.txt
```
...
Full results in `reports/SCF_v1_3_REPORT.md`; everything below is switchable, and the
...
```bash
build/01_saturation_tree_induction/scf_real_audit --input data/real/tokenized.txt \
...
Full results in `reports/SCF_v1_4_REPORT.md`. v1.4 replaces the global yield
...
```bash
build/01_saturation_tree_induction/scf_audit v14-synthetic --output-dir results/v1_4/synthetic
build/01_saturation_tree_induction/scf_real_audit --input data/real/tokenized.txt \
