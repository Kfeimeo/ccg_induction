...
## 57. v2.3.1 clean-corpus module (`scf::v231`)

`include/scf/clean_corpus.hpp` / `src/clean_corpus.cpp` is a thin layer over `scf::v23`: it never touches `observe_sentences` or `ConservativeMerger`. It supplies (a) two corpus loaders producing the same `sentences + token_text` input the v2.3 driver builds, and (b) diagnostics computed from the merger's public state (`accepted()`, `rejected()`, `class_of`, `data.witnesses`, `data.context_keys`). The structured loader reads the `.scs` format written by `tools/prepare_clean_corpus.py` (`#doc`, `#par`, one raw sentence per line), tokenizes with the v2.1 tokenizer, keeps internal punctuation, and pops the trailing `. ? !` run as the `<EOS>` signal. Sentences never cross a paragraph or document boundary because segmentation happens per source line in the Python step and every sentence is observed on its own. The `v23d` loader is a line-for-line copy of the v2.3 driver's sentence construction (condition-A nominal counts for prefix selection, condition-D actual counts) so its numbers are checked against the untouched `scf_conservative_merging` CLI.

## 58. Frame-type classification

Trie root 0 is epsilon in both context tries, so `(left == 0, right == 0)` classifies a context as `empty_frame`, `left_boundary`, `right_boundary` or `internal_frame` with no string comparison. A candidate's type is the type of its recorded witness (the first witness per unordered pair in sorted witness order — the same `try_emplace` rule the merger uses to pick the context it stores in `MergeRecord` / `ConflictRecord`). A separate "exclusive" count reports candidates whose witnesses are all of one type; for the `(eps, eps)` hub these coincide in practice because a pair of complete-sentence spans almost never shares any other exact frame. The largest-class contribution counts accepted merges whose pair lies in the largest final class.

## 59. Corpus determinism

Both fetches are fixed byte ranges / fixed row groups of a fixed file, recorded with sha256 in the report. peS2o `text` has a documented layout (title, abstract, body paragraphs separated by blank lines); title and abstract are dropped by position, section headers are kept because no source field separates them from paragraph text. FineWeb has no structure fields, so each newline-separated line is a paragraph. The sentence segmenter is a fixed regular expression with a fixed abbreviation list; no length, frequency, POS or model-based filtering is applied anywhere.
