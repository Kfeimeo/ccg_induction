...
# v2.3.1 additions (clean-corpus replication)

## 57. What is and is not shared with v2.3

`scf::v231` (include/scf/clean_corpus.hpp, src/clean_corpus.cpp) links `scf_conservative_v23` and calls `v23::observe_sentences`, `v23::ConservativeMerger`, and `v23::compare_partitions` unchanged; no file under the v2.3 module was edited. The module owns only (a) two corpus readers, (b) the nested-prefix rule, (c) the frame-type diagnostics, and (d) the audit writers. The `v23_condition_d` reader is a line-for-line copy of the corpus construction inside `v23::run_conservative_scaling` (v2.1 tokenizer, `segment_sentences`, punctuation dropped, prefixes in condition-A tokens, the same `1.25 * N + 2e6` read limit), so running the v2.3.1 tool in that mode on the v2.3 input must reproduce the v2.3 CSV row exactly; `tools/compare_v231.py` asserts this for every shared column before it builds a comparison table. That identity is the proof that the comparison isolates the corpus.

## 58. Structure-preserving reader

Input is one body paragraph per line with an empty line between documents (`tools/extract_wiki_body.py --mode body`). Tokens come from the v2.1 tokenizer (`v21::tokenize_line`: lowercase, digit runs to `<num>`, single visible ASCII punctuation as one-character tokens, 2+ apostrophes as separators). Sentences are segmented inside a paragraph at every `. ? !` token exactly as v2.2/v2.3 do; the terminator is consumed (it is the `<EOS>` observation, not a lexical token), a paragraph end closes the open sentence, and a document boundary is a paragraph boundary. All other punctuation tokens are kept (`--punctuation drop` reproduces the condition-D removal on the structured corpus, used only as a supplementary attribution run). Because `v23::observe_sentences` enumerates spans per sentence and frames are exact full-sentence remainders, no object, frame, or witness can cross a sentence or paragraph boundary; the test suite checks this on adjacent paragraphs. Nominal and actual token counts coincide in this mode (nothing is removed after segmentation); the prefix at scale N is the shortest whole-sentence prefix with >= N tokens, which nests by construction.

## 59. Frame types are read off the trie roots

`v23::ObservedDataset` encodes the exact frame as a pair of trie node ids whose root 0 is the empty sequence, so `frame_type_of` is `(left == 0, right == 0)` with no string handling. A candidate pair usually has one witness; when it has several, the pair carries a 4-bit type mask and the diagnostics report both `any` (mask bit set) and `only` (mask is a single bit) counts, with `mixed` as the complement so the `only` rows plus `mixed` partition the candidate table exactly. The merger records the representative context of each accepted/rejected candidate; the audit files print that context together with the full type mask. `#{u : (eps,eps) observed for u}` is the number of objects whose sorted context list contains the (0,0) context id, which equals the number of distinct complete sentences of length <= max_len.

## 60. Hub measurement without the learner

`empty_frame_hub_stats` inserts every sentence of length <= max_len into a set per nested prefix and reports d = |set| and C(d,2). Every pair of distinct complete short sentences is a direct (eps,eps) witness, so C(d,2) is a lower bound on the candidate table the unchanged v2.3 learner must process. On the structured corpus d grows from 1,180 (1e5) to 33,823 (1e7), i.e. from 7e5 to 5.7e8 candidates, and the measured runtime law of the unchanged learner (see the v2.3.1 report) makes 1e6 and 1e7 resource-bound; per v2.3 section 7 the bound is reported and no frequency/length filter is introduced to hide it.
